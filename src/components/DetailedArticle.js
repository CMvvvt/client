...
const defaultNYTimesImgUrl =
  "https://upload.wikimedia.org/wikipedia/commons/0/0e/Nytimes_hq.jpg";
const defaultGuardianImgUrl =
  "https://assets.guim.co.uk/images/eada8aa27c12fe2d5afa3a89d3fbae0d/fallback-logo.png";
>>>>>>> e87b8c4546f0dabf77fb48a8c57afe0ff3397470
...
class DetailedArticle extends React.Component {
  ...
  componentDidMount() {
    const queryParams = qs.parse(
      this.props.pageLocation.search.replace("?", "")
    );
    const { id } = queryParams;
    this.removeCommentBox = commentBox("5750073610731520-proj", {
      defaultBoxId: id,
  ...
  handleClickFavorite = (
    ...
    if (window.localStorage.getItem(sharingUrl) === null) {
      ...
      window.localStorage.setItem(sharingUrl, JSON.stringify(article));
      this.setState({});
      let marked = toast("Saving " + title, {
        position: "top-center",
        bodyClassName: CSS({ color: "black" }),
      });
      setTimeout(() => {
        toast.dismiss(marked);
      }, 2000);
    } else {
      window.localStorage.removeItem(sharingUrl);
      this.setState({});
      let unmark = toast("Removing " + title, {
        position: "top-center",
        bodyClassName: CSS({ color: "black" }),
      });
      setTimeout(() => {
        toast.dismiss(unmark);
      }, 2000);
    }
    // this.setState((prevState) => ({
    //     favorite:
  ...
  buildArticlePage = (apiMode, article) => {
    ...
    if (this.props.article !== null) {
      if (apiMode === "NYTimes") {
        // console.log(multimedia);
        const { news_desk, web_url, headline, pub_date, multimedia, abstract } =
          article;
        let image = multimedia.find((img) => {
          try {
            return img.width >= 2000 && img.url !== null;
        ...
        const imageUrl = isEmptyObj(image)
          ? defaultNYTimesImgUrl
        ...
        const { sectionId, id, webUrl, webTitle, blocks, webPublicationDate } =
          article;
        let imageUrl = "";
        try {
          let assets = blocks["main"]["elements"][0]["assets"];
            ...
            <Container fluid>
              <Row>
                ...
                <Col sm={1} xs={5} style={{ position: "relative" }}>
                  ...
                  <EmailShareButton
                    children={
                      <EmailIcon size={24} round={true} data-tip={"Email"} />
                    }
                    subject={"#CSCI_571_NewsApp"}
                    url={sharingUrl}
                  />
  ...
  showChevron = () => {
    return this.state.iconFlipped ? (
      <FaChevronUp
        style={{ float: "right" }}
        onClick={this.handleClickChevron}
      />
    ) : (
      <FaChevronDown
        style={{ float: "right" }}
  ...
  buileLoader = (isLoading) => {
    if (isLoading) {
      return (
        <Container
          fluid={"true"}
          style={{ margin: "24% auto", textAlign: "center" }}
        >
          <BounceLoader
            css={overrideLoader}
            size={40}
